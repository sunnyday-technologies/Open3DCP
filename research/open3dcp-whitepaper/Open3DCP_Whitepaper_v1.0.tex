...
ingestion} on six openly-licensed public datasets --- a cast-concrete
benchmark, a corporate carbon-aware set, an ultra-high-performance
(UHPC) corpus, a \textasciitilde30-laboratory 3DCP mechanical database,
an extrusion-3DCP printability set, and a project-layer building
catalogue --- into one schema, including a cross-study print-anisotropy
result. The point Open3DCP makes is one of \textbf{characterization, not
prediction}: each source records a different slice of the material and
process --- composition, fresh-state rheology, hardened performance,
embodied carbon, print orientation, or the as-built project --- and the
schema holds their union in one shape, so that what any one study leaves
...
\item
  A \textbf{worked demonstration} ingesting six openly-licensed public
  datasets --- a cast-concrete benchmark, a corporate carbon-aware set,
  a UHPC corpus, a \textasciitilde30-laboratory 3DCP mechanical
  database, an extrusion-3DCP printability set, and a project-layer
  building catalogue --- into one schema, three of them carrying a real
  automated fidelity score, and yielding a cross-study print-anisotropy
  result that the schema's orientation field makes expressible (§6).
\end{itemize}
...
\textbf{Worked demonstration: six public datasets in one schema.} To
show the schema works on real data --- not only by design --- we
ingested six openly-licensed public datasets into Open3DCP, chosen so
that each lights up a \emph{different} slice of the record (Figure 5,
left). Three are flat kg/m³ tables that the \texttt{open3dcp-ingest}
tool converts and scores automatically:
...
\item
  ...
  \textbf{98.8/100 (A)} (126 populated cells across its 9 fields) ---
  the normal-strength reference point.
\item
  ...
  ingests at \textbf{98.9/100 (A)} and additionally populates
  \textbf{embodied carbon} (per-mix GWP) and the measurement-uncertainty
  columns. It makes a cross-source trade-off legible in one schema: a
  plain-Portland concrete at \textasciitilde521 kg CO2/m³ for
  \textasciitilde65 MPa beside a low-clinker ternary at
  \textasciitilde204 kg CO2/m³ for \textasciitilde110 MPa --- higher
  strength at roughly 60 \% less embodied carbon. (Beyond this measured
  table, the same repository ships a trained Gaussian-process model and
  a model-explored design space of optimization-derived candidate mixes;
  recording such \emph{model-generated} candidates alongside
  \emph{measured} data --- kept distinct by Open3DCP's
  \texttt{measurement\_confidence} flag --- is a natural next ingestion,
  not attempted here.)
\item
  \textbf{UNSW Global UHPC} {[}50{]}, an ultra-high-performance corpus,
  ingests at \textbf{98.2/100 (A)} and exercises the high-strength end
  the others do not reach --- silica fume, steel fibre, and the
  flexural, splitting-tensile, and elastic-modulus columns across 28-day
  compressive strengths of \textasciitilde130--281 MPa (with early-age
  points down to \textasciitilde81 MPa).
\end{itemize}

These scores are best read as an \emph{ingestion-completeness} check on
a curated excerpt, not a dataset-quality grade: for dense flat kg/m³
tables, field coverage is \textasciitilde100 \% by construction, so the
grade is driven by the value-fidelity term (here, only the
superplasticizer solids-fraction assumption), and the scoring convention
itself is proposed, not yet calibrated (§10).
...
\item
  The \textbf{UF/UTK 3D-Printing-Concrete Mix-Design} dataset {[}51{]}
  supplies the extrusion-3DCP \emph{printability} slice --- static and
  dynamic yield stress and plastic viscosity --- and is an honest
  illustration of a basis mismatch: it reports constituents as ratios to
  binder with no absolute kg/m³, so a constituent mass-\% of the mix is
  left NULL rather than invented.
\item
  The \textbf{TU-Braunschweig Database of 3D Concrete Printed Buildings}
  {[}52{]} supplies the \emph{project / process layer} --- as-built
...
A seventh UHPC source surveyed for this work, the Stevens dataset
(Mahjoubi \& Bao), is cited reference-only: its mix variables are
published as a coded ML feature matrix whose legend is paywalled, so it
cannot be honestly re-ingested. Together the six light up complementary
slices, and Open3DCP holds the union of these six sources in one shape
--- and \emph{the empty cells are as much the point as the full ones}:
what each source cannot supply is recorded, not hidden (Figure 5, left).
...
\texttt{test\_orientation\_code} (X/Y/Z/CAST), the same printed mortar
is \textbf{32--55 \% weaker loaded along the layers} than the cast
reference, consistently across three independent laboratories (Figure 5,
right). (These are flexural results, where loading along the print path
bends the interlayer planes, so the along-layer weakness is consistent
with the interface being the weak link; the ``typical'' strength
ordering of Table 1 is mix- and loading-dependent, as Limitation 4
notes.) This is a demonstration of \emph{ingestion and interoperability}
--- recording what was measured in one comparable shape --- not a
predictive-model benchmark; assembling and modelling a pooled
multi-source corpus is future work (§11).

\begin{figure}
\centering
\includegraphics[width=6.4in,height=\textheight,keepaspectratio,alt={Worked demonstration. Left: six openly-licensed public datasets ingested into Open3DCP --- UCI/Yeh (cast benchmark), Meta SustainableConcrete (cast, carbon-aware), UNSW UHPC, the RILEM TC 304-ADC interlaboratory study (real 3DCP, \textasciitilde30 labs), the UF/UTK 3DCP mix-design set (printability), and the TU-Braunschweig 3DCP buildings catalogue (project layer) --- each populating a different slice of the record (dark = populated, light = partial/absent), with Open3DCP spanning the union; the empty cells are recorded, not hidden. Three kg/m³ sources carry an automated open3dcp-ingest fidelity score (98.2--98.9 / A); the rest are curated through their native structure. Right: a cross-study characterization result from the RILEM data, expressible only because the schema records loading orientation --- printed concrete loaded along the layers (X) is 32--55 \% weaker than the cast reference, consistently across three independent laboratories (flexural strength, \textasciitilde28 d; error bars are ± one standard deviation).}]{figures/fig5_demonstration.png}
\caption{\textbf{Worked demonstration.} \emph{Left:} six openly-licensed
public datasets ingested into Open3DCP --- UCI/Yeh (cast benchmark),
Meta SustainableConcrete (cast, carbon-aware), UNSW UHPC, the RILEM TC
304-ADC interlaboratory study (real 3DCP, \textasciitilde30 labs), the
UF/UTK 3DCP mix-design set (printability), and the TU-Braunschweig 3DCP
...
Three kg/m³ sources carry an automated \protect\texttt{open3dcp-ingest}
fidelity score (98.2--98.9 / A); the rest are curated through their
native structure. \emph{Right:} a cross-study characterization result
from the RILEM data, expressible only because the schema records loading
orientation --- printed concrete loaded along the layers (X) is 32--55
\% weaker than the cast reference, consistently across three independent
laboratories (flexural strength, \textasciitilde28 d; error bars are ±
...
\tightlist
\item
  \textbf{Schema \& tooling:} Open3DCP v1.7 {[}45{]} ---
...
\item
  ...
  License.
\item
  U. J. Malik, C. K. Lee, D. Mohotti, H. Mo (2025). \emph{A global
  dataset of UHPC mix designs with supplementary cementitious materials,
  nano additives, and sustainable fillers} (V2) {[}Dataset{]}. Mendeley
  Data. \url{https://doi.org/10.17632/czb7ww5pkz.2} (CC BY 4.0).
\item
  J. Gao, Z. Wang, C. Wang (2022). \emph{3D Printing Concrete Mix Design
  Open Dataset} (v0.3) {[}Dataset{]}. University of Florida. Zenodo.
  \url{https://doi.org/10.5281/zenodo.6828947} (CC BY 4.0).
\item
  G. Placzek, M. Dahlberg (2024). \emph{Database of 3D Concrete Printed
  Buildings} {[}Dataset{]}. Zenodo.
  \url{https://doi.org/10.5281/zenodo.14214812} (CC BY 4.0).
\end{enumerate}
